...
FROM docker.io/fedora:35 as fedora-guardserver
RUN dnf clean all && dnf -y install bash zsh wireguard-tools iptables iputils iproute net-tools bash systemd iputils iproute socat zsh bind-utils openssh-clients openssh-server procps-ng
COPY --from=fedora-guard-builder /guard /usr/bin/guard
RUN chmod 0700 /usr/bin/guard
RUN mkdir -p /etc/wireguard
